...
# .github/workflows/church_calendar_sync.yml

```yaml
name: Sync Church Calendar to Google Calendar
    ...
    steps:
      ...
      - name: Install Python dependencies
        run: |
          ...
          pip install requests selenium google-api-python-client google-auth google-auth-httplib2 google-auth-oauthlib

      - name: Run church calendar sync
        env:
          LCR_USERNAME: ${{ secrets.LCR_USERNAME }}
          LCR_PASSWORD: ${{ secrets.LCR_PASSWORD }}
          GOOGLE_CALENDAR_ID: ${{ secrets.GOOGLE_CALENDAR_ID }}
          GOOGLE_SERVICE_ACCOUNT_JSON: ${{ secrets.GOOGLE_SERVICE_ACCOUNT_JSON }}
          CHURCH_INCLUDE_HIDDEN: "false"
          SYNC_DAYS_BACK: "14"
          SYNC_DAYS_FORWARD: "180"
          HEADLESS: "true"
        run: python church_calendar_sync.py
```
...
3. Copy the service account JSON key into the `GOOGLE_SERVICE_ACCOUNT_JSON` GitHub secret.
4. Share your ward calendar with the service account email as **Make changes to events**.
5. Use the calendar's `@group.calendar.google.com` address as `GOOGLE_CALENDAR_ID`.

---

# Notes

* This uses `includeHiddenCalendars=false` by default.
* It ignores `eventTranslations` automatically because it only uses the top-level event fields.
* It stores the church event ID in Google `extendedProperties.private.churchSourceId` so later runs update instead of duplicating.
* Blank address parts and null event-contact values are skipped.
* Timed events use `start.dateTime` / `end.dateTime`; all-day events use `start.date` / `end.date`, matching Google Calendar API expectations.
